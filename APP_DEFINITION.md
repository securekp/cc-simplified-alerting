...
**Implication:** a coverage table built only from the group-level config calls **silently omits every Pack-defined feed**, including ones moving real volume. This is a genuine coverage blind spot, not a cosmetic gap. `openapi.json` exposes `/p/{pack}/system/inputs`, `/p/{pack}/system/outputs`, and `/p/{pack}/notifications`, and `GET /notifications` takes an `includePacks` flag — so the data is reachable. **MVP decision: Pack-scoped feeds are explicitly out of scope, and the app must say so in the UI** (a note on the coverage summary stating that feeds defined inside Packs are not shown), rather than presenting an incomplete table as if it were complete. Covering Packs moves to the deferred list.
...
* **Pack-qualified feeds still carry traffic.** `datagen:cribl-palo-alto-networks.palo_traffic`, `open_telemetry:observeai.observeai-claudeDesktopOTLP`, `open_telemetry:observeai.observeai-claudeCodeCLI` — none has a row in `/system/inputs`. The MVP disclosure note stays required.
...
6. App fetches existing Notifications (`GET /notifications`) and the monitors from the host group, matching each back to the Source or Destination it watches so coverage can be shown per row. Bridge Notifications (`monitor-{id}`) are set aside from that matching and used to say whether each monitor delivers anywhere.
7. App renders Sources and Destinations as separate sections, each row carrying: name, type, group, current health, error indicator, and **one coverage column** answering "is anything watching this feed for a delivery stop?"
...
* **Existing alerts** — `GET /notifications` (accepts a **`groupId`** filter, plus `includePacks`), matched back to the feed each targets, to compute the coverage column. See "Matching Existing Alerts Back to Feeds" below — this is not a trivial lookup.
...
**Creates nothing else.** Two Cribl objects, both of them Cribl's own, and no third mechanism.
...
* **Never modifies** Sources, Destinations, pipelines, routes, notification targets, notification policies, or Insights settings. The app reads platform configuration and writes only alerting configuration. Insights settings are not written — and no longer read either, since the column that needed them is gone.
...
* **Pack-scoped feeds** — not shown, and the coverage summary says so explicitly rather than presenting a partial count as the whole deployment.
...
A record per alert the app created, holding its ID, the worker group, the feed ID, its direction, the signal it watches, and the template settings used. For a monitor the settings also record `mechanism: 'monitor'`, the host group it was written to, the template it copied its query from, and its bridge's ID.
...
Reconciliation requirement: the registry is a cache, not the truth. On load the app cross-checks each entry against **the read for its own mechanism** — Notifications against `GET /notifications`, monitors against the host group's monitor collection — and drops entries whose object no longer exists, so stale entries never make an unwatched feed look watched. Reconciling monitor entries against Notifications alone would delete every one of them on load and report the app's own monitors as unmanaged. And when **no host group answered**, monitor entries are not judged at all: they would look stale for exactly the same reason they look invisible, and the app would delete its own record of a monitor that is still there. An alert that exists but isn't in the registry still counts toward coverage — it is just shown as unmanaged.
...
* **Page every list endpoint to exhaustion** (`/notifications`, `/conditions`), always sending `offset` alongside `limit` and validating the envelope.
* **Match existing alerts back to feeds** on `(group, condition category, conf.name)`. Report unattributable alerts as such rather than guessing, including the genuinely ambiguous groupless-name-in-two-groups case.
...
* State plainly in the UI that **Pack-scoped feeds are not covered**, so an incomplete table is not mistaken for a complete one.
...
* Coverage table split into Sources and Destinations, each row carrying name, type, group, current health, a **separate error indicator** so a Green-with-error feed is visibly not fine, and **one coverage column**.
...
* **Pack-scoped feed coverage** — `/p/{pack}/system/inputs`, `/p/{pack}/system/outputs`, `/p/{pack}/notifications`, and `includePacks` on `GET /notifications`. Verified finding 9 shows these feeds carry real traffic and real health, so this is the most substantive coverage gap on the deferred list.
...
* **Pack-scoped Sources and Destinations** for MVP — disclosed in the UI as a stated limitation, not silently omitted (Verified finding 9).
...
* Sources and Destinations in separate sections, each headed with an "N unwatched" count.
