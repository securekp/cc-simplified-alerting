...
**A feed inside a Pack gets both mechanisms, exactly like a group feed.** `mechanismFor` in `plan.ts` reads the direction's selected mechanism and nothing else — scope is not part of that answer. Scope changes two things, and both are addressing rather than choice: a Notification goes to the Pack's own collection (`POST /m/:gid/p/:pack/notifications`), and **both ids carry a Pack segment** (`alertId`, and `monitorIdFeedPart` for a monitor).

It was Mechanism A only for as long as the tag was unverified, and the reason it no longer is deserves recording, because it is the evidence that retired a documented block. Rule 3 above pins a monitor's per-feed scope in `rules[0].includedTags`, a tag matching no series is a monitor that is created successfully, charts an empty line and never fires, and a Pack feed's dimension value had been observed **both** Pack-qualified and bare across different metrics — so the standing rule was *block, do not improvise*. Read live on **2026-09-03**, splitting the throughput metrics (the family every shipped query measures) by `input` and by `output`:

- Pack feeds appear **only** Pack-qualified — `datagen:cribl-palo-alto-networks.palo_traffic`, `open_telemetry:observeai.observeai-claudeDesktopOTLP`, `cribl_lake:observeai.observeai-claudeDesktopLake`, `cribl_search_engine:observeai.observeai-mainSearch`.
- **No bare-id series exists for any of them.** In particular `cribl_lake:observeai-claudeDesktopLake` is not in the metric store, which retires the one observation the ambiguity rested on. It must have come from a different metric family, so the caution is now narrower and still worth keeping: **the tag form is a property of the metric family**, and the only family that matters is the one the copied query reads.
- Splitting by `namespace` adds no such key to any series, so `namespace` is **not a dimension these metrics carry**. The shipped `{namespace=""}` matcher therefore treats a Pack feed exactly as it treats a group feed — there is no asymmetry for a Pack to fall through.

`feedTag` already returned the right string (`feed.metricKey`, which is `type:pack.id`), so nothing about the payload had to be invented to lift this — which is the point of having built `feedMetricKey` scope-aware in the first place.

**Confirmed by a real write, not only by the tag read.** A Source monitor and a Destination monitor were created for feeds inside a Pack on the verification org (2026-09-03) and both landed. That matters as its own line of evidence: the tag read proved the series exists, and only a write proves the monitor collection accepts a Pack-bearing id and that the object shows up where an admin goes looking for it.
...
## Packs Are a Second Scope, Not a Second Capability

A Pack is a scope *inside* a worker group, and its Sources and Destinations are genuinely invisible from the group-level calls: a Source defined inside a Pack has **no row** in `/m/:gid/system/inputs`. An earlier build therefore rendered a table that read as complete while a Pack feed moving 33 MB an hour was absent from it, disclosed only as a note. That disclosure is gone because the gap is closed, not because the gap stopped mattering.

Every path is group-prefixed *and then* Pack-prefixed — `/m/:gid/p/:pack/…`, the Pack prefix always inside the group prefix, because Packs are installed per worker group. `api/feeds.ts` takes a `FeedScope` (`{group, pack}`) rather than a group id, so one function reads both scopes and every rule that governs a group feed governs a Pack feed unchanged.

- **The Pack list is `GET /m/:gid/packs`, paged like everything else.** `toPacks` filters on `isDisabled !== true` — a disabled Pack still reports its contents, so its own state is the enabled test, never the feeds inside it — and keeps `inputs`/`outputs` as `number | null`. Those counts are **permission to skip a request, never data**: `packHasFeeds` answers `false` only on a *reported* zero, because the fields are optional in the schema and reading "did not say" as "has none" would hide a whole Pack's feeds on no evidence, which is the exact failure this feature exists to fix.
- **`pack` is stamped from the scope the payload was read in, never parsed out of the entries.** The Pack-level call returns feeds keyed on their **bare** id exactly as the group-level call does, so nothing in the response distinguishes the two.
- **A Pack's alerts are read one Pack at a time**, from `GET /m/:gid/p/:pack/notifications` — *not* `includePacks=true` on the group call. A Notification carries a bare `conf.name` and nothing naming a Pack, so which collection returned it is attribution's only handle on scope; a flat merged list would make a Pack alert on `palo_traffic` and a group alert on `palo_traffic` indistinguishable, and the never-guess rule would then drop both. The scope travels beside the object as `ScopedNotification {raw, pack}` rather than being stamped onto API data Cribl never returned. Dedupe **against the group-level ids only**: the Pack-context list accepts its own `groupId`/`includePacks` and may hand back group-level alerts, but two Packs each holding the same id are both kept, because their scope tells them apart.
- **Scope is part of the identity in both directions.** A Pack alert never attributes to a group feed and a group alert never reaches inside a Pack (`(feed.pack ?? null) === pack`, exact). Without that, one Source called `palo_traffic` in a Pack and another in its group would make both alerts ambiguous and cost real coverage.
- **`alertId` appends a Pack segment only for a Pack feed**, so every id already written keeps its exact spelling and the KV registry keeps resolving. It has to be there for a Pack feed, though: one id for both scopes would make the second write an edit of the first.
- **`feedRowId` always emits a Pack segment, empty for a group feed.** A Pack and its group are two rows with two coverage answers.
- **A Pack's `default` and `devnull` Destinations are dropped** — `UNALERTABLE_PACK_TYPES` in `lib/feeds.ts`, matched on `type` and applied only when `pack !== null`. Every Pack ships both whether it uses one or not, so a dozen Packs contribute two dozen rows to which the app's one question cannot be put: a `devnull` discards by design and a `default` forwards to another Destination rather than delivering anywhere itself. Left in, they are most of what a Pack section's "N unwatched" count is counting, which turns the number an admin is meant to act on into noise. Matched on `type` rather than `id` because being a non-delivery sink is what makes a feed unalertable, not what it is called — a second DevNull under a custom id goes too. **Group scope is untouched**: there is one of each there rather than one per Pack, so there is no volume of noise to justify hiding a real row from an audit.
- **Packs are deliberately not a fifth capability.** There are still four. A group whose Pack list is denied loses its Pack rows with a note naming the group; a Pack whose alert collection is unreadable has its registry entries **not judged** rather than pruned — an unreadable collection is not evidence the object is gone, the same treatment a monitor entry gets when no host answered. A Pack that is genuinely *gone* **is** judged: its alerts went with it.
- **`indexFeedIdentities` includes Pack feeds, keyed on the Pack-aware id segment.** It skipped them while a Pack feed could not get a monitor, because `sanitizeIdPart` sees only the bare id and a Pack Source named `palo_traffic` would have made `collisionBlock` refuse the *group* feed's monitor over one the app was never going to write. Now that a Pack feed does get a monitor it has to be indexed, and the key is `monitorIdFeedPart(feed.id, feed.pack)` — the exact string the monitor id is built from, called through the same function rather than recomposed, because an index keyed differently from the id it is checking finds no clash and lets the silent overwrite through.
...
2. **Join on `type` + `id`, and match exactly.** Metric dimension values are fully qualified `type:id` (`syslog:in_syslog:udp`, `cribl_lake:palo_traffic`) while config objects key on the bare `id`. Construct `` `${type}:${id}` `` from the config object and compare for **exact equality** — never parse the dimension value apart, and never prefix-match. Verified live: a feed emits both a parent series and `:suffix` children (`subscription:zscaler_web_project` and `...:zscaler`) carrying the **same** value, so prefix matching double-counts. There is also an undimensioned roll-up series to exclude, and the metric store holds internal feeds (`ServiceTcp:*`, `cribl:CriblLogs`, `router:*`) plus Pack-qualified feeds (`type:packName.feedId`) that have no config row at all. **The config call is the spine; metrics join onto it, never the reverse.**
   - **A Pack feed's dimension value is `type:pack.id`** — `datagen:cribl-palo-alto-networks.palo_traffic` carried 33 MB in an hour while no `datagen:palo_traffic` series existed at all. `feedMetricKey` takes the Pack as a third argument and every consumer reads `feed.metricKey` rather than recomposing `type:id`, because the two agreed only while every feed was group-level.
...
1. **Review alert coverage** — landing table of every enabled Source and Destination, in the worker groups *and* inside the Packs installed in them, with current health, a separate error indicator, and one coverage column. An unhealthy feed with nothing watching it is the most important row in the app; style it that way.
...
- **Installed Packs, and the feeds inside them** — `GET /m/:gid/packs` (filtered to `isDisabled !== true`), then `GET /m/:gid/p/:pack/system/inputs` and `/system/outputs` for each Pack that did not report a count of zero for that direction. Identical payload to the group-level call, so `status.health` and `status.error` arrive the same way. See "Packs Are a Second Scope" above for why this is not optional and why the counts are permission-to-skip rather than data.
- **Health detail (progressive enhancement)** — `GET /m/:gid/system/status/inputs`, `GET /m/:gid/system/status/outputs`, and their Pack-scoped equivalents under `/m/:gid/p/:pack/system/status/…`. Adds `healthCounts` per state across Worker Processes, an `error` object, `pq` queue status, `timestamp`. Fetch lazily on row expand, not for the base table, and cache per `(group, pack, direction)` — a Pack's status list is a different response, and keying without the Pack would hand a Pack row the group's list.
...
- Existing alerts — `GET /notifications` (takes a `groupId` filter) plus `GET /m/:gid/p/:pack/notifications` once per Pack, and only the collections: coverage and the configuration view both read the stored object out of those lists, so there is no per-alert read and no item path is granted. Matching these back to feeds is non-trivial: a Notification carries only a bare `conf.name`, so resolve direction from the condition's `category` and match on `(group, pack, direction, id)` — the Pack coming from which collection answered, not from the object. A groupless Notification whose name matches feeds in two groups is genuinely ambiguous — **report it as unattributed rather than counting it toward both.** Never guess.
...
**What the app writes:** Notifications — at the group level, or inside a Pack for a Pack feed — Insights monitors, and its own KV state. Nothing else: never Sources, Destinations, pipelines, routes, notification targets, notification policies, Insights settings, or **a Pack itself** (the app installs, edits and removes none). The app creates alerts; it does not delete them, including its own, so no DELETE is granted anywhere. `/notifications` and `/m/:gid/p/:pack/notifications` are each granted GET and POST and have **no item path at all**; `/m/:gid/alert/monitors/*` is granted PATCH alone, and **only** as the recovery when a POST is rejected because that id already exists — never as a blind first move, which would overwrite an object this app did not create.
...
- `cc-simplified-alerting/managed/{notification|monitor}/{id}` — registry of alerts this app created (group, feed id, direction, signal, and the template settings, which record the `mechanism` and, for a monitor, its `hostGroup`, `templateId` and bridge id). It is what lets the coverage column distinguish "created here" from "unmanaged" without inferring ownership from names. Keyed on the created object's id, so a monitor entry is keyed on the monitor id, not the bridge's. When there is no entry, ownership falls back to a marker the app wrote into the object itself — the reserved `csa-` id namespace for a Notification, `APP_MONITOR_MARK` in the `description` for a monitor — never to a name that looks familiar.
  ...
  - **A Notification record carries the `pack` it was written into**, so a later load knows which collection to reconcile it against. Its ids are not in the group-level list, so without that every Pack entry would be judged stale on the next load and the app would delete its own record of an alert that is still there. A Pack whose list or whose alert collection could not be read is **not judged**, the same as a monitor entry when no host answered; a Pack that is genuinely gone **is** judged, because its alerts went with it.
...
- **Four sections, each with its own "N unwatched" count: Sources, Destinations, Sources in Packs, Destinations in Packs.** Direction has to be the heading — the two have different health semantics, different metrics and an asymmetric condition catalogue — and scope is one too, though **not** because the options differ — they do not, a Pack row takes whichever mechanism the admin picked. Scope is a heading because a Pack is a collection an admin reasons about separately and so deserves its own "N unwatched" count, and because a `Pack` column would be blank in every group-level row. The Pack sections carry that column; the group sections do not.
...
- **The mechanism is a per-direction choice in the drawer, and only appears when there is a choice.** One mechanism usable → no selector, and a notice saying which one and why. Neither → nothing is creatable, said once. **Scope does not enter this**: one selector and one form per direction, whatever the mix of scopes, because a Pack feed takes the mechanism its direction is set to. Scope used to fork it — a Pack feed was always a Notification, so a direction holding both scopes needed a monitor form and a condition form at once, with the condition select relabelled "Condition for the feeds inside a Pack". That is gone, and it should stay gone: two forms and a scope-qualified label made an identical option set read as two different features, which is what an admin actually complained about.
...
There are four capabilities: `alerting`, `monitors`, `routingTargets`, `registry`. Only `alerting` and `monitors` can block a write, and they do so independently — each blocks items of its own mechanism and nothing else. **Packs are deliberately not a fifth**: a Pack write is a Notification write, so it lives and dies with `alerting`, and a Pack whose *reads* fail costs that Pack's rows and nothing more.
...
- One group's Pack list denied, or one Pack's feeds unreadable → that scope's rows are missing and nothing else is. One notice per distinct problem, naming the group and, where it is one Pack rather than the list, the Pack — because it is a different collection, not a different formatting of the same one — and saying the table is **incomplete rather than wrong**. The group's own feeds are unaffected.
- One Pack's alert collection unreadable → coverage inside that Pack is unknown, said once, and its registry entries are **not judged** so the app never prunes its own record of an alert that is still there.
...
**Must-have (Phase 1):** worker-group discovery filtered to `type === 'stream'`; Pack discovery per group filtered to `isDisabled !== true`, with the feeds inside each Pack read from its own collection; Source/Destination discovery filtered to `disabled !== true`; correct `type` + `id` → `type:id` metric-dimension join, Pack-qualified as `type:pack.id` inside a Pack; runtime discovery of Notification conditions with schema-driven forms per direction; monitor host discovery by probe with shipped-query templates per direction; coverage table with health, a separate error indicator, and one coverage column carrying both mechanisms, in four scope-and-direction sections; group/direction/uncovered/unhealthy/has-an-error filters and a search that includes the Pack; multi-select with "select all uncovered"; bulk-apply drawer with a per-direction mechanism choice and a routing choice between a notification policy and targets-with-a-template; mandatory preview showing every object to be written; sequential creation with progress, per-item results including partial success, the created object, and retry; click-through configuration view for any attributed alert of either mechanism; KV registry and template defaults with per-mechanism reconciliation; graceful degradation.
